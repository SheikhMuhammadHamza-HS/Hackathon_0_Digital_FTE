#!/usr/bin/env python3
"""
Odoo Test Setup - Configure and test Odoo connection
"""
...
"""
    ...
    if not env_file.exists():
        ...
        print(f"  📝 Please edit {env_file} with your Odoo credentials")
    else:
...
def test_odoo_connection():
    ...
    try:
        ...
        async def run_test():
            ...
            if server_info:
                print(f"  ✅ Connected to Odoo {server_info.get('version', 'unknown')}")
                print(f"     Database: {server_info.get('database')}")
                print(f"     URL: {server_info.get('server_url')}")
            else:
                print("  ⚠️  Connected but couldn't retrieve server info")
...
"""
        ...
        print(f"  ℹ️  Test client already exists")
...
def main():
    ...
    if not check_odoo_requirements():
        ...
        print("  2. Run: python test_odoo_setup.py")
        return False
    ...
    if success:
        ...
        print("\n❌ Odoo setup failed. Please check configuration.")
